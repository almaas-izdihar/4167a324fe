...
# Uses colab --auth=adc ls (file op) — safe, does not interrupt running kernel.
...
while true; do
    ...
    echo "-- sessions --"
    colab --auth=adc sessions 2>&1

    echo "-- models dir --"
    colab --auth=adc ls "/content/ema-skd/models/" -s "$SESSION" 2>&1 || echo "(not available yet)"

    echo "-- log tail --"
    colab --auth=adc ls "/content/ema-skd/models/" -s "$SESSION" 2>/dev/null \
        | grep -oE '[^ ]+' \
        | head -1 \
        | xargs -I{} colab --auth=adc ls "/content/ema-skd/models/{}/log/" -s "$SESSION" 2>/dev/null \
